...
users.update = function (id, users, result) {
  dbConn.query("UPDATE users SET firstname=?, lastname=?, email=?, placeofbirth=?, birthdate=?, country=?, current_location=?, timezone=?, phone=?, status=? WHERE id = ?", 
  [users.firstname, users.lastname, users.email, users.placeofbirth, users.birthdate, users.country, users.current_location, users.timezone, users.phone, users.status, id], function (err, res) {
    if (err) {
      console.log("error: ", err);
      result(null, err);
    } else {
